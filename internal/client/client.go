...
func New(cfg *config.Config, timeout time.Duration, rateLimit float64) *Client {
	...
	cacheDir := filepath.Join(homeDir, ".cache", "cloudbeds-pp-cli")
...
func (c *Client) do(method, path string, params map[string]string, body any, headerOverrides map[string]string) (json.RawMessage, int, error) {
	...
	for attempt := 0; attempt <= maxRetries; attempt++ {
		...
		if req.Header.Get("User-Agent") == "" {
			req.Header.Set("User-Agent", "cloudbeds-pp-cli/v1.3")
		}
		...
		// Unauthorized - try refreshing the access token once, then retry
		if resp.StatusCode == 401 && attempt == 0 && c.Config != nil && c.Config.RefreshToken != "" {
			if err := c.refreshAccessToken(); err == nil {
				if newHeader := c.Config.AuthHeader(); newHeader != "" {
