...
In local mode: searches locally synced data only.`,
  ...
  cloudbeds-pp-cli search "error timeout"

  # Force local search only
  cloudbeds-pp-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  cloudbeds-pp-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  cloudbeds-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'cloudbeds-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			case "get_email_templates":
				results, err = db.SearchGetEmailTemplates(query, limit)
			case "get_items":
				results, err = db.SearchGetItems(query, limit)
			case "":
				// Search all FTS-enabled tables individually to avoid duplicates.
				seen := make(map[string]bool)
				_ = seen // prevent unused error when no FTS tables exist
				{
					partial, searchErr := db.SearchGetEmailTemplates(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search get_email_templates failed: %w", searchErr)
					}
				...
				{
					partial, searchErr := db.SearchGetItems(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search get_items failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
