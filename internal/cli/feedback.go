...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".cloudbeds-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("CLOUDBEDS_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("CLOUDBEDS_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "cloudbeds-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.cloudbeds-pp-cli/feedback.jsonl.
When ` + "`CLOUDBEDS_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`CLOUDBEDS_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  cloudbeds-pp-cli feedback list
  cloudbeds-pp-cli feedback list --limit 5
  cloudbeds-pp-cli feedback list --json`,
